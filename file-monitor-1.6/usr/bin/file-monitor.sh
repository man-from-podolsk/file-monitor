...
#===================================================================================================
# Скрипт: file-monitor.sh
# # Версия 1.6 (с поддержкой конфига)
# Назначение: Мониторинг целостности системных файлов через auditd.
#             Настройки вынесены в /etc/file-monitor/file-monitor.conf
#             Поддерживает hot-reload по SIGHUP
#             Валидирует конфиг перед применением.
#             Ротация лога по достижении 10Mb, хранятся 10 копий, переопределить можно в файле
#             /etc/logrotate.d/file-monitor
# Версия 1.5
# Назначение: Мониторинг целостности системных файлов через auditd.
#             Создаёт правила аудита, кэширует файлы, логгирует изменения (содержимое, метаданные).
#
# Особенности:
#   - Все значения по умолчанию вверху (редактируемая секция).
#   - Автосоздание/перезапись audit-правил при каждом запуске.
#   - Не использует внешний config.conf.
#   - Обрабатывает wildcard (*), рекурсивно сканирует директории.
#   - Фильтрует временные файлы (.swp, ~, .rpmnew и др.). и файлы readme
#   - Ограничивает diff по размеру.
#   - Использует augenrules --load.
# Зависимости:
#   auditd, ausearch, coreutils, bash 4+
# Логирование:
#   - В файл, указанный в LOG_FILE
#
# Автор: [Алексей Рыбянец]
# Дата: 23.07.2025 + обновление 28.11.2025
#===================================================================================================
...
# Устанавливаем обработчик до всего
